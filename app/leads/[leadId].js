...
const ViewPage = () => {  
    const {leadId, SalesPerson ,InterestLevel ,Source, Cellular, CardName, MaxLocalTotal } = useLocalSearchParams();
    const navigation = useNavigation();
    // console.log('Lead ID:', leadId);
    // console.log('SalesPerson:', SalesPerson, 'InterestLevel:', InterestLevel, 'Source:', Source, 'Cellular:', Cellular, 'CardName:', CardName);
  ...
  try {
    ...
    // Initialize follow-up info
    if (salesData.SalesOpportunitiesLines.length > 0) {
      const lastEntry = salesData.SalesOpportunitiesLines[salesData.SalesOpportunitiesLines.length - 1];

      setRemarks(salesData.SalesOpportunitiesLines.map(line => line.Remarks || ''));
      setFollowUpDate(lastEntry.ClosingDate?.split('T')[0] || '');
      setStartDate(lastEntry.ClosingDate?.split('T')[0] || '');
...
const safeValue = (val) =>
  val === null || val === undefined || val === 'null' ? 'N/A' : String(val);
  ...
  const canUpdateLead = () => {
    ...
    const hasPermission = role === 'sale_head' || role === 'telecaller' || 
                         storedEmpCode === Number(salesOpportunity.SalesPerson);
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <Text style={styles.infoValue}>
              Sequence No {leadId || 'N/A'}
            </Text>
            <Text style={styles.cardSubtitle}>
              Created: {salesOpportunity?.StartDate ? formatDate(salesOpportunity?.StartDate) : 'N/A'}
            </Text>
          </View>
